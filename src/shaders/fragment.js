...
void main() {
    ...
    vec3 stem_color = vec3(.15 + u_stop_randomizer[0] * .3, .5 + u_stop_randomizer[1] * .2, .2);
    
    // Romantic flower color palette
    float colorChoice = fract(u_stop_randomizer[0] * 5.0);
    vec3 flower_color;
    
    if (colorChoice < 0.2) {
        // Pink
        flower_color = vec3(1.0, 0.4, 0.6);
    } else if (colorChoice < 0.4) {
        // Red
        flower_color = vec3(0.9, 0.2, 0.3);
    } else if (colorChoice < 0.6) {
        // Purple/Violet
        flower_color = vec3(0.7, 0.3, 0.9);
    } else if (colorChoice < 0.8) {
        // White/Cream
        flower_color = vec3(1.0, 0.95, 0.9);
    } else {
        // Hot pink
        flower_color = vec3(1.0, 0.2, 0.5);
    }
    
    // Add slight variation
    flower_color += vec3(u_stop_randomizer[1] * 0.1 - 0.05);
